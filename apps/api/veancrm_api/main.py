...
from veancrm_api.db import get_async_session as core_get_async_session
from veancrm_api.middleware.security import add_security_middleware
...
# -----------------------------------------------------------------------------
# БД: основная (VEANCRM) - с улучшенной обработкой ошибок
# -----------------------------------------------------------------------------
if not settings.VEANCRM_DB_URL:
    raise RuntimeError("VEANCRM_DB_URL is not configured")

try:
    engine_veancrm: AsyncEngine = create_async_engine(
        settings.VEANCRM_DB_URL,
        echo=False,
        pool_pre_ping=True,
    ...
    SessionVeancrm = sessionmaker(engine_veancrm, class_=AsyncSession, expire_on_commit=False)
except Exception as e:
    logger.error(f"Failed to create primary database engine: {e}")
    raise

async def get_veancrm_session() -> AsyncGenerator[AsyncSession, None]:
    async with SessionVeancrm() as session:
        try:
            yield session
        except Exception as e:
...
# -----------------------------------------------------------------------------
# БД: клиентская (удалена для CRM) - с улучшенной обработкой
# -----------------------------------------------------------------------------
# CRM система использует только одну БД
    try:
...
async def on_startup():
    ...
    for attempt in range(max_retries):
        try:
            async with engine_veancrm.begin() as conn:
    ...
    # CRM система использует только одну БД
    logger.info("VEANCRM database warmup completed.")
    
    # Глобальная подмена зависимости
    app.dependency_overrides[core_get_async_session] = get_veancrm_session
...
async def readiness():
    ...
    try:
        async with engine_veancrm.begin() as conn:
            await conn.run_sync(lambda *_: None)
        checks["veancrm_db"] = {"status": "healthy", "type": "postgresql"}
    except Exception as e:
        checks["veancrm_db"] = {"status": "unhealthy", "error": str(e)[:100]}
...
# --- CRM роутеры ---
from veancrm_api.routes import (
    auth_crm,
    clients_crm,
    appointments_crm,
    transactions_crm,
...
# Подключение CRM роутеров
app.include_router(auth_crm.router, prefix=PREFIX)
app.include_router(clients_crm.router, prefix=PREFIX)
app.include_router(appointments_crm.router, prefix=PREFIX)
app.include_router(transactions_crm.router, prefix=PREFIX)
