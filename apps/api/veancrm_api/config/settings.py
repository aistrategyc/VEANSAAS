# apps/api/veancrm_api/config/settings.py
...
class Settings(BaseSettings):
    ...
    VEANCRM_DB_URL: Optional[str] = None  # ОБЯЗАТЕЛЬНО через .env файл
    ...
    JWT_ISSUER: str = "veancrm"
    JWT_AUDIENCE: str = "veancrm-clients"
    ACCESS_TTL_SEC: int = 15 * 60
    REFRESH_TTL_SEC: int = 30 * 24 * 60 * 60
    ACCESS_TOKEN_EXPIRE_MINUTES: Optional[int] = None
    REFRESH_COOKIE_NAME: str = "vcrm_refresh"
    ...
    EMAIL_FROM: str = "no-reply@veancrm.com"
    FRONTEND_URL: str = "https://app.veancrm.com"
    ...
    def _backward_compat(cls, values: dict):
        if not values.get("VEANCRM_DB_URL") and values.get("POSTGRES_URL"):
            values["VEANCRM_DB_URL"] = values["POSTGRES_URL"]
    ...
    @field_validator("VEANCRM_DB_URL")
    @classmethod 
