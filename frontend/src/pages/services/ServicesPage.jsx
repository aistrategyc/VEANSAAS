...
export default function ServicesPage() {
  ...
  const handleSaveService = (serviceData) => {
    if (selectedService) {
      setServices((prev) =>
        prev.map((service) => (service.id === selectedService.id ? { ...service, ...serviceData } : service)),
      )
    } else {
      const newService = {
        id: `serv${Date.now()}`,
  ...
  const handleDeleteService = (serviceId) => {
    setServices((prev) => prev.map((service) => (service.id === serviceId ? { ...service, isActive: false } : service)))
  }
  ...
  const handleSaveCategory = (categoryData) => {
    if (selectedCategory) {
      setCategories((prev) =>
        prev.map((category) => (category.id === selectedCategory.id ? { ...category, ...categoryData } : category)),
      )
    } else {
      const newCategory = {
        id: `cat${Date.now()}`,
  ...
  const handleDeleteCategory = (categoryId) => {
    setCategories((prev) =>
      prev.map((category) => (category.id === categoryId ? { ...category, isActive: false } : category)),
    )
  }
  ...
  const stats = {
    ...
    averageDuration: Math.round(services.reduce((sum, s) => sum + s.duration, 0) / services.length),
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Управление услугами и категориями салона</p>
          </div>
          ...
          <TabsContent value="services" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Найдите нужные услуги по различным критериям</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                .map((category) => {
                  ...
                  return (
                    <Card key={category.id} className="cursor-pointer hover:shadow-md transition-shadow">
                      ...
                      <CardContent>
                        <div className="flex items-center justify-between">
                          <Badge variant="secondary">{categoryServices.length} услуг</Badge>
                          <div className="text-sm text-muted-foreground">
                            от {Math.min(...categoryServices.map((s) => s.price)).toLocaleString()} ₽
                          </div>
                        </div>
                      </CardContent>
                    </Card>
